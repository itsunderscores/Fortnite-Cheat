...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool tab, IDirect3DTexture9* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    auto anim = button_anim.find(id);

    if (anim == button_anim.end())
    {
        button_anim.insert({ id, 0.0f });
        anim = button_anim.find(id);
    }
    ...
    ImColor bg_col(12, 12, 12, 255);
    ImColor text_col(255, 255, 255, 100 + int(155 * anim->second));
    ImColor icon_col(150, 200, 255, 100 + int(155 * anim->second));

    draw_list->AddRectFilled(bb.Min, bb.Max, bg_col, 5);
    if (tab) {
        ImVec2 min = ImVec2(bb.Min.x + 5, bb.Min.y + size_arg.y / 2 - 10);
        ImVec2 max = ImVec2(bb.Min.x + 25, bb.Min.y + size_arg.y / 2 + 10);
        draw_list->AddImage(icon, min, max, ImVec2(0, 0), ImVec2(1, 1), icon_col);
        draw_list->AddText(ImVec2(bb.Min.x + 40, bb.Min.y + size_arg.y / 2 - label_size.y / 2),
            text_col, label);
    } 
    else {
        draw_list->AddText(ImVec2(bb.Min.x + size_arg.x / 2 - label_size.x / 2, bb.Min.y + size_arg.y / 2 - label_size.y / 2),
            text_col, label);
    }
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, bool tab, IDirect3DTexture9* icon)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, tab, icon);
}
...
struct checkbox_anim_struct {
    float first;
    float second;
    float third;
};
static std::map<ImGuiID, checkbox_anim_struct> checkbox_anim;

bool ImGui::Checkbox(const char* label, bool* v, bool settings, IDirect3DTexture9* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    ImDrawList* draw_list = GetWindowDrawList();
    ...
    auto find_anim = checkbox_anim.find(id);
    if (find_anim == checkbox_anim.end()) {
        checkbox_anim.insert({ id, { 0.0f, 0.0f, 0.0f } });
        find_anim = checkbox_anim.find(id);
    }
    checkbox_anim_struct& anim = find_anim->second;

    const ImVec2 item_size(250.0f - anim.third, 50.0f);
    const ImVec2 pos = window->DC.CursorPos;
    ...
    if (checked) {
        if (settings && anim.third < 100)
            anim.third += 5.0f;
        if (anim.second < 1.0f)
            anim.second += 0.05f;
    }
    else {
        if (settings && anim.third > 0)
            anim.third -= 5.0f;
        if (anim.second > 0.0f)
            anim.second -= 0.05f;
    }

    const ImRect check_bb(pos, pos + item_size);
    const bool mixed_value = (g.LastItemData.InFlags & ImGuiItemFlags_MixedValue) != 0;

    if (is_visible)
    {
        ImColor bg_col(12, 12, 12, 255);
        ImColor box_col(150, 200, 255, 100 + int(75 * anim.first) + int(155 * anim.second));
        ImColor text_col(255, 255, 255, 100 + int(75 * anim.first) + int(155 * anim.second));
        ImColor mark_col(12, 12, 12, int(255 * anim.second));

        // Draw settings icon (if enabled)
        if (settings && anim.third > 1.0f)
        {
            ImColor bg_col2(12, 12, 12, int(255 * (anim.third / 100)));
            ImColor text_col2(255, 255, 255, int(255 * (anim.third / 100)));

            ImVec2 min = ImVec2(check_bb.Max.x - 50, check_bb.Min.y);
            ImVec2 max = ImVec2(check_bb.Max.x, check_bb.Min.y + 50);
            draw_list->AddRectFilled(min, max, bg_col2, 5.0f);

            ImVec2 min2 = ImVec2(min.x + 15, min.y + 15);
            ImVec2 max2 = ImVec2(min.x + 35, min.y + 35);
            draw_list->AddImage(icon, min2, max2, ImVec2(0, 0), ImVec2(1, 1), text_col2);

            if (ImGui::IsWindowFocused() &&
                mouse_pos.x > min.x && mouse_pos.x < max.x &&
        ...
        // Draw background
        draw_list->AddRectFilled(check_bb.Min, check_bb.Max, bg_col, 5.0f);

        // Draw checkbox square
        const float box_size = 20.0f;
        ImVec2 box_pos = ImVec2(check_bb.Min.x + 10.0f, check_bb.Min.y + (item_size.y - box_size) * 0.5f);
        ImVec2 box_max = box_pos + ImVec2(box_size, box_size);
        draw_list->AddRectFilled(box_pos, box_max, box_col, 5.0f);

        // Draw checkmark if checked
        if (checked) {
            ImVec2 check_pos = box_pos + ImVec2(3, 3);
            RenderCheckMark(draw_list, check_pos, mark_col, box_size - 6.0f);
        }

        // Draw label
        ImVec2 label_pos = ImVec2(box_max.x + 10.0f, check_bb.Min.y + item_size.y / 2.0f - label_size.y / 2.0f);
...
struct begincombo_anim_struct {
    float first;
    float second;
};
static std::map<ImGuiID, begincombo_anim_struct> begincombo_anim;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(420, 50));
    ...
    auto find_anim = begincombo_anim.find(id);

    if (find_anim == begincombo_anim.end())
    {
        begincombo_anim.insert({ id, { 0.0f, 0.0f } });
        find_anim = begincombo_anim.find(id);
    }

    begincombo_anim_struct& anim = find_anim->second;
    ...
    ImColor bg_col(12, 12, 12, 255);
    ImColor text_col(255, 255, 255, 100 + int(150 * anim.first));
    ImU32 arrow_col = GetColorU32(ImVec4(150.0f / 255.0f, 200.0f / 255.0f, 1.0f, 0.4f + (0.4f * anim.first)));

    window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), bb.Max, bg_col, 5);

    RenderArrow(window->DrawList, ImVec2(bb.Min.x + 382, bb.Min.y + 18), arrow_col, ImGuiDir_Down, 1.0f);
    ...
    {
        ImVec2 preview_value_size = ImGui::CalcTextSize(preview_value);
        draw_list->AddText(ImVec2(bb.Min.x + 210 - preview_value_size.x / 2, bb.Min.y + 25 - preview_value_size.y / 2),
            text_col, preview_value);
    }

    draw_list->AddText(ImVec2(bb.Min.x + 20, bb.Min.y + 25 - label_size.y / 2), text_col, label);
...
struct slider_anim_struct {
    float first;
    float second;
    float third;
    float menu_pos;
};
static std::map<ImGuiID, slider_anim_struct> slider_anim;
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(250, 50));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    auto find_anim = slider_anim.find(id);

    if (find_anim == slider_anim.end())
    {
        slider_anim.insert({ id, { 0.0f, 0.0f, 0.0f, ImGui::GetWindowPos().x } });
        find_anim = slider_anim.find(id);
    }

    slider_anim_struct& anim = find_anim->second;
    ...
    grab_bb.Min.x = frame_bb.Min.x + 2;
    anim.first = grab_bb.Max.x;

    if (anim.menu_pos != ImGui::GetWindowPos().x) {
        anim.second = anim.first;
    }
    else {
        anim.second += (anim.first - anim.second) * 0.1f; 
    }

    anim.menu_pos = ImGui::GetWindowPos().x;
    grab_bb.Max.x = anim.second;
    ...
    ImColor bg_col(12, 12, 12, 255);
    ImColor box_col(150, 200, 255, 100 + int(155 * anim.third));
    ImColor mark_col(12, 12, 12, int(255 * anim.second));
    ImColor text_col(255, 255, 255, 100 + int(155 * anim.third));

    draw_list->AddRectFilled(frame_bb.Min, frame_bb.Max, bg_col, 5);

    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, box_col, 4);
    }
    ...
    {
        float x_min = frame_bb.Min.x + 210 - value_buf_size.x / 2;
        float y_min = frame_bb.Min.y + 25 - value_buf_size.y / 2;
        draw_list->AddText(ImVec2(x_min, y_min), text_col, value_buf, value_buf_end);
    }
    {
        float x_min = frame_bb.Min.x + 20;
        float y_min = frame_bb.Min.y + 25 - label_size.y / 2;
        draw_list->AddText(ImVec2(x_min, y_min), text_col, label);
    }
...
struct selectable_anim_struct {
    float first;
    float second;
    bool close;
};
static std::map<ImGuiID, selectable_anim_struct> selectable_anim;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const bool span_all_columns = false;
    const float min_x = span_all_columns ? window->ParentWorkRect.Min.x : pos.x;
    const float max_x = span_all_columns ? window->ParentWorkRect.Max.x : window->WorkRect.Max.x;
    if (size_arg.x == 0.0f || (flags & ImGuiSelectableFlags_SpanAvailWidth))
        size.x = ImMax(label_size.x, max_x - min_x);

    const ImVec2 text_min = ImVec2(pos.x + 15, pos.y);
    ...
    auto find_anim = selectable_anim.find(id);

    if (find_anim == selectable_anim.end())
    {
        selectable_anim.insert({ id, { 0.0f, 0.0f, false } });
        find_anim = selectable_anim.find(id);
    }

    selectable_anim_struct& anim = find_anim->second;
    ...
    if (is_visible)
    {
        ...
        ImColor select_col(150, 200, 255, int(150 * anim.first));
        ImVec2 min = ImVec2(bb.Min.x + 10, bb.Min.y);
        ImVec2 max = ImVec2(bb.Max.x - 10, bb.Max.y);
        draw_list->AddRectFilled(min, max, select_col, 5);
    }

    if (is_visible) {
        ImColor text_col(255, 255, 255, 100 + int(155 * anim.first));
        draw_list->AddText(text_min, text_col, label);
    }
