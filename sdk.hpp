...
{
	Camera view_point{};
	uintptr_t location_pointer = driver.readdriver<uintptr_t>(cache::uworld + 0x140);
	uintptr_t rotation_pointer = driver.readdriver<uintptr_t>(cache::uworld + 0x150);
	FNRot fnrot{};
	fnrot.a = driver.readdriver<double>(rotation_pointer);
	fnrot.b = driver.readdriver<double>(rotation_pointer + 0x20);
	fnrot.c = driver.readdriver<double>(rotation_pointer + 0x1D0);
	view_point.location = driver.readdriver<Vector3>(location_pointer);
	view_point.rotation.x = asin(fnrot.c) * (180.0 / M_PI);
	view_point.rotation.y = (((atan2(fnrot.a * -1, fnrot.b) * (180.0 / M_PI)) * -1) * -1);
	view_point.fov = driver.readdriver<float>(cache::player_controller + 0x740) * 90.f;
	//view_point.fov = driver.readdriver<float>(cache::player_controller + 0x740) * settings::fov_value;
	return view_point;
}
...
Vector2 project_world_to_screen(Vector3 world_location)
{
	...
	if (settings::Features::ultrawidefix)
	{
		return Vector2(
			settings::screen_center_x + vtransformed.x * (((settings::screen_center_x - 1080) / tanf(cache::local_camera.fov * M_PI / 360))) / vtransformed.z,
			settings::screen_center_y - vtransformed.y * (((settings::screen_center_x - 1080) / tanf(cache::local_camera.fov * M_PI / 360))) / vtransformed.z
		);
	}

	else if (settings::Features::changeresolution) {
		//allow them to change the resolution sizing
		return Vector2(
			settings::get_screen_center_x() + vtransformed.x * ((settings::get_screen_center_x2() / tanf(cache::local_camera.fov * M_PI / 360))) / vtransformed.z,
			settings::get_screen_center_y() - vtransformed.y * ((settings::get_screen_center_x3() / tanf(cache::local_camera.fov * M_PI / 360))) / vtransformed.z
		);
	}
	else {
		//return regular size
...
{
	uintptr_t bone_array = driver.readdriver<uintptr_t>(mesh + BONE_ARRAY);
	if (bone_array == 0) bone_array = driver.readdriver<uintptr_t>(mesh + BONE_ARRAY_CACHE);
	FTransform bone = driver.readdriver<FTransform>(bone_array + (bone_id * 0x60));
	FTransform component_to_world = driver.readdriver<FTransform>(mesh + COMPONENT_TO_WORLD);
...
{
	auto Seconds = driver.readdriver<double>(cache::uworld + 0x2e0); //0x148 could it be 0x2e0 ??
	auto LastRenderTime = driver.readdriver<float>(mesh + 0x32c); //0x30C
	return Seconds - LastRenderTime <= 0.05f;
}
